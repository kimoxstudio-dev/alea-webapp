---
name: kx-no-ai-slop
description: Edit prose into sharper, more human writing while preserving the writer's voice, or name the AI-slop patterns in a draft without rewriting it. Use when writing or revising anything that ships to a reader outside the session — a post, an email, landing copy, README prose — when a draft sounds AI-written, or when asked to audit one. Session output follows kx-style instead.
---

<!-- Generated by kx-sync from skills/no-ai-slop in kx.
...
The word lists above are English. The patterns all translate. The Spanish
versions met most often: "En el mundo de", "En la era de", "cabe destacar",
"es importante destacar", "sumérgete en", "no solo X sino que también Y",
"la clave está en", "En conclusión". Cut them the same way — state the
point, name the source, end on the concrete fact.
